...
if (isset($_SESSION['user'])) :
    ...
    //------------------------------ Insertion dans la base de donnée si toutes les infos sont bonnes ------------------------------//
        if (empty($problem)) {
            ...
            do {
                $fileName1 = uniqid('image_');
            } while (file_exists($path . $fileName1 . $extension1));

            move_uploaded_file($_FILES['photo1']['tmp_name'], $path . $fileName1 . '.' . $extension1);
            ...
            $statement->bindValue(':thumnail', $fileName1 . '.' . $extension1, PDO::PARAM_STR);
